...
## Programa Target API (v2)

### Already Present in Programa
...
6. `browser.tab.*` compatibility aliases mapped to Programa surfaces
7. `browser.console.list`
8. `browser.errors.list`
9. `browser.highlight`
10. `browser.state.save|load` (browser state in Programa context)
...
Primary form:
```bash
programa browser --surface <surface-id> <agent-browser-style-command...>
```

Shorthand:
```bash
programa browser <surface-id> <agent-browser-style-command...>
```

Agent discovery:
```bash
programa identify
programa capabilities
programa browser identify --surface <surface-id>   # wrapper over system.identify + browser fields
```

Flash:
```bash
programa trigger-flash [--workspace <id>] [--surface <id>]
```
...
- [x] Add `browser` command group in `CLI/programa.swift` that accepts agent-browser-style command grammar.
...
3. `src/protocol.test.ts` -> adapted browser command/shape validation in `tests_v2/test_browser_api_unsupported_matrix.py` and existing `CLI/programa.swift` command grammar checks.
...
5. `src/daemon.test.ts`, `src/stream-server.test.ts`, `test/serverless.test.ts`, `src/ios-manager.test.ts` -> out-of-scope for Programa browser parity (different transport/runtime).

### Implemented Programa Browser Suites
...
1. `programa browser tab ...` maps to browser `surface` tabs only (no separate workspace-level tab meaning inside `browser` namespace).
...
5. Output shape is Programa-native overall, but `browser.snapshot` and selector `not_found` diagnostics intentionally mirror agent-browser semantics for agent usability.
...
2. Whether to expose protocol-only agent-browser actions in first public release of `programa browser` or gate them behind a second rollout phase.
