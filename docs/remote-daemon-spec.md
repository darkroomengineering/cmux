...
Tracking issue: https://github.com/darkroomengineering/programa/issues/151
Primary PR: https://github.com/darkroomengineering/programa/pull/1296
CLI relay PR: https://github.com/darkroomengineering/programa/pull/374
...
- `DONE` `programa ssh` creates remote-tagged workspaces and does not require `--name`.
- `DONE` scoped shell niceties are applied only for `programa ssh` launches.
...
- `DONE` local app probes remote platform, verifies a release-pinned `programad-remote` artifact by embedded manifest SHA-256, uploads it when missing, and runs `serve --stdio`.
...
- `DONE` relay address written to `~/.programa/socket_addr` on the remote only after the reverse forward survives startup validation.
...
1. `DONE` `programad-remote` now supports proxy stream RPC (`proxy.open`, `proxy.close`, `proxy.write`, `proxy.stream.subscribe`) with pushed `proxy.stream.data/eof/error` events.
...
|---|---|---|---|
| M-001 | `programa ssh` workspace creation + metadata + optional `--name` | DONE | Covered by `tests_v2/test_ssh_remote_cli_metadata.py` |
...
| M-004b | CLI relay: run programa commands from within SSH sessions | DONE | Reverse TCP forward + Go CLI relay + bootstrap wrapper |
...
| ID | Scenario | Status |
|---|---|---|
| C-001 | `programa ping` from remote session | DONE |
| C-002 | `programa list-workspaces --json` from remote | DONE |
| C-003 | `programa new-workspace` from remote | DONE |
| C-004 | `programa rpc system.capabilities` passthrough | DONE |
...
1. `PROGRAMA_SSH_TEST_DOCKER_HOST` sets the SSH destination host/IP used by docker-backed SSH fixtures (default `127.0.0.1`).
2. `PROGRAMA_SSH_TEST_DOCKER_BIND_ADDR` sets the bind address used in fixture container publish mappings (default `127.0.0.1`).
