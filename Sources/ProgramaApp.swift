...
// Settings/About titlebar styling. Release-reachable: SettingsAboutTitlebarStyleStore is
// applied by SettingsRootView.configureSettingsWindow (SettingsView.swift) and by
// AboutWindowController/SettingsWindowController below, so it cannot live in
// DebugWindows.swift, which is entirely #if DEBUG. The DEBUG-only tuning UI
// (SettingsAboutTitlebarDebugWindowController) still reads/writes this store from
// DebugWindows.swift. `localizedDebugLabel` is intentionally duplicated (private,
// file-scoped) rather than shared, to avoid adding a new file.
...
@MainActor
final class SettingsAboutTitlebarStyleStore: ObservableObject {
    static let shared = SettingsAboutTitlebarStyleStore()
