...
type focusedContext struct {
	...
	windowId    string
	paneHandle  string
	// paneId is the canonicalized pane UUID, resolved via tmux_target.go's
	// selector matching. Falls back to paneHandle when canonicalization
	// fails or times out. Used (not paneHandle) as the source for the fake
	// TMUX_PANE token so the id an agent sees matches what tmux-compat's
	// format context later emits for the same pane.
	paneId    string
	surfaceId string
}
...
// canonicalizeFocusedContextWithTimeout resolves ctx.paneId to the
// canonical pane UUID in the background, bounded by timeout so a
// slow/stale relay can't block agent launch. Returns the base context
// unchanged if canonicalization doesn't finish in time.
