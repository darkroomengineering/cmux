...
// runCLI is the entry point for the "cli" subcommand (or busybox "programa" invocation).
func runCLI(args []string) int {
	socketPath := os.Getenv("PROGRAMA_SOCKET_PATH")
	...
	for i := 0; i < len(args); i++ {
		...
		case "--socket":
			if i+1 >= len(args) {
				fmt.Fprintln(os.Stderr, "programa: --socket requires a path")
				return 2
			}
			socketPath = args[i+1]
			i++
...
doneFlags:
	...
	if socketPath == "" {
		fmt.Fprintln(os.Stderr, "programa: PROGRAMA_SOCKET_PATH not set and --socket not provided")
		return 1
	}
	...
	if !ok {
		fmt.Fprintf(os.Stderr, "programa: unknown command %q\n", cmdName)
		return 2
	}
...
func runRPC(socketPath string, args []string, jsonOutput bool, refreshAddr func() string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "programa rpc: requires a method name")
		return 2
	}
	method := args[0]
	...
	if len(args) > 1 {
		if err := json.Unmarshal([]byte(args[1]), &params); err != nil {
			fmt.Fprintf(os.Stderr, "programa rpc: invalid JSON params: %v\n", err)
			return 2
		}
	}
...
// runBrowserRelay handles "programa browser <subcommand>" by mapping to browser.* v2 methods.
func runBrowserRelay(socketPath string, args []string, jsonOutput bool, refreshAddr func() string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "programa browser: requires a subcommand (open, navigate, back, forward, reload, get-url)")
		return 2
	}
	...
	case "get-url":
		...
		useSurfaceEnv = true
	default:
		fmt.Fprintf(os.Stderr, "programa browser: unknown subcommand %q\n", sub)
		return 2
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "programa browser: %v\n", err)
		return 2
	}
...
func applyWorkspaceEnvFallback(params map[string]any) {
	...
	if envWs := os.Getenv("PROGRAMA_WORKSPACE_ID"); envWs != "" {
...
func applySurfaceEnvFallback(params map[string]any) {
	...
	if envSf := os.Getenv("PROGRAMA_SURFACE_ID"); envSf != "" {
...
// readSocketAddrFile reads the socket address from ~/.programa/socket_addr as a fallback
// when PROGRAMA_SOCKET_PATH is not set. Written by the programa app after the relay establishes.
func readSocketAddrFile() string {
	home, err := os.UserHomeDir()
	if err != nil {
		return ""
	}
	data, err := os.ReadFile(filepath.Join(home, ".programa", "socket_addr"))
	if err != nil {
		return ""
	}
...
func readRelayAuthFile(socketPath string) *relayAuthState {
	if strings.Contains(socketPath, ":") && !strings.HasPrefix(socketPath, "/") {
		...
		data, err := os.ReadFile(filepath.Join(home, ".programa", "relay", port+".auth"))
		if err != nil {
			return nil
		}
...
func currentRelayAuth(socketPath string) *relayAuthState {
	relayID := strings.TrimSpace(os.Getenv("PROGRAMA_RELAY_ID"))
	relayToken := strings.TrimSpace(os.Getenv("PROGRAMA_RELAY_TOKEN"))
...
// dialSocket connects to the programa socket. If addr contains a colon and doesn't
...
func authenticateRelayConn(conn net.Conn, auth *relayAuthState) error {
	...
	if challenge.Protocol != "programa-relay-auth" || challenge.Version != 1 || challenge.RelayID != auth.RelayID || challenge.Nonce == "" {
...
func cliUsage() {
	fmt.Fprintln(os.Stderr, "Usage: programa [--socket <path>] [--json] <command> [args...]")
	...
	fmt.Fprintln(os.Stderr, "  omo [args...]              Launch OpenCode with programa integration")
	fmt.Fprintln(os.Stderr, "  omx [args...]              Launch Oh My Codex with programa integration")
	fmt.Fprintln(os.Stderr, "  omc [args...]              Launch Oh My Claude Code with programa integration")
